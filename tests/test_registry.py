...
from daybetter_local_api.device_registry import DeviceRegistry
from daybetter_local_api.device import DayBetterDevice
from daybetter_local_api.controller import DayBetterController


class TestDeviceRegistry(unittest.TestCase):
    def setUp(self):
        self.registry = DeviceRegistry()
        self._mock_controller = Mock(spec=DayBetterController)
        self.device = DayBetterDevice(
    ...
    def test_discovery_custom_device(self):
        self.registry.add_device_to_queue("192.168.1.2")
        DayBetterDevice(self._mock_controller, "192.168.1.2", "device2", "sku2", None)
        assert "192.168.1.2" in self.registry.devices_queue
