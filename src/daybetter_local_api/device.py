...
from .light_capabilities import DayBetterLightCapabilities, ON_OFF_CAPABILITIES
from .message import DevStatusResponse


class DayBetterSegment:
    ...
    def __str__(self) -> str:
        return f"<DayBetterSegment is_on={self.is_on}, color={self.color}>"
...
class DayBetterDevice:
    def __init__(
        ...
        fingerprint: str,
        sku: str,
        capabilities: DayBetterLightCapabilities = ON_OFF_CAPABILITIES,
    ) -> None:
        ...
        self._capabilities: DayBetterLightCapabilities = capabilities

        self._is_on: bool = False
        self._rgb_color = (0, 0, 0)
        self._temperature_color = 0
        self._brightness = 0
        self._update_callback: Callable[[DayBetterDevice], None] | None = None
    ...
    @property
    def capabilities(self) -> DayBetterLightCapabilities:
    ...
    @property
    def update_callback(self) -> Callable[[DayBetterDevice], None] | None:
        return self._update_callback

    def set_update_callback(
        self, callback: Callable[[DayBetterDevice], None] | None
    ) -> Callable[[DayBetterDevice], None] | None:
    ...
    def __str__(self) -> str:
        result = f"<DayBetterDevice ip={self.ip}, fingerprint={self.fingerprint}, sku={self.sku}, lastseen={self._lastseen}, is_on={self._is_on}"
        return result + (
